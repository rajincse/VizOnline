...
public class ViewerCreator extends JDialog {
    ...
    /**
     * Viewer creator to enable offline creation of viewers
     *
     * @param env
     * @param dataSourceName
     * @param viewerName
     */
    public ViewerCreator(Environment env, String defaultName, String dataSourceName, String viewerName) {

      
        currentViewerFactory = null;
        //    System.out.println("The size of the viewerFactories is "+ env.getViewerFactories().size());
        ViewerFactory vf = null;
        for (int i = 0; i < env.getViewerFactories().size(); i++) {
            vf = env.getViewerFactories().get(i);
       ...
       // int index = env.getDataSources().indexOf(dataSourceName);
