...
public class BundledGraph extends GraphViewer {
	...
	public BundledGraph(String name, GraphData g) {
		...
		Property<PInteger> p = new Property<PInteger>("bundle",new PInteger(0));	
		try {
			this.addProperty(p);
		} catch (Exception e) {
		...
		Property<PInteger> pp = new Property<PInteger>("smooth",new PInteger(0));
	...
	public <T extends PropertyType> void propertyUpdated(Property p, T newvalue) {
		...
		if (p.getName() == "bundle")
		{
			if (eb == null) eb = new ForceDirectedEdgeBundler(this);
			eb.compute();
		}
		if (p.getName() == "smooth")
		{
